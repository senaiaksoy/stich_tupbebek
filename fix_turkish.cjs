...
const BACKUP = 'D:/1web/stitch_tupbebek/public/e-kitap/tup-bebek-beslenme-plani.backup.html';
const OUT = 'D:/1web/stitch_tupbebek/public/e-kitap/tup-bebek-beslenme-plani.html';

let content = fs.readFileSync(BACKUP, 'utf8');
const styleEnd = content.indexOf('</style>');
const cssPart = content.substring(0, styleEnd + 8);
let htmlPart = content.substring(styleEnd + 8);
...
// Apply replacements
let count = 0;
for (const [old, nw] of Object.entries(R)) {
  while (htmlPart.includes(old)) {
    htmlPart = htmlPart.replace(old, nw);
    count++;
  }
}

fs.writeFileSync(OUT, cssPart + htmlPart, 'utf8');
console.log(`Done! Applied ${count} replacements from ${Object.keys(R).length} patterns.`);
